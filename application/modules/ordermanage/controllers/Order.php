...
class Order extends MX_Controller {
	...
	public function posprintview($id){
		...
		$data_fel = [];
		$data_fel["order_id"] = $id;
		$items = $this->order_model->customerorder($id);

		foreach ($items as $key => $value){
			$data_fel ["items"][$key]["ProductName"] = $value->ProductName . " " . $value->variantName;
			$data_fel ["items"][$key]["precio_unidad"] = $value->mprice ;
			$data_fel ["items"][$key]["precio_total"] = $value->mprice * $value->menuqty;
			$data_fel ["items"][$key]["menuqty"] = $value->menuqty;
		}

		if($customerorder->cutomertype ==  1) {
			$propina_total = $grandtotal_new * 0.1;
			$key = $key +1;
			$data_fel ["items"][$key]["ProductName"] = "Propina";
			$data_fel ["items"][$key]["precio_unidad"] = $propina_total;
			$data_fel ["items"][$key]["precio_total"] =  $propina_total;
			$data_fel ["items"][$key]["menuqty"] = 1;
		}
		// Agregamos NIT
		$data_fel["nit"] = $settinginfo->vattinno;
		
		$data_x = json_encode($data_fel);

		$URL = "https://app.centraldepago.com/api/certificar";
		$ch = curl_init($URL);
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER,
			array(
				'Content-Type:application/json',
				'Content-Length: ' . strlen($data_x)
			) );
		curl_setopt($ch, CURLOPT_POSTFIELDS, "$data_x");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		$output = curl_exec($ch);
		$output_fel = json_decode($output , true);
      
		//var_dump($data_x);die;
      
		$data['AUTORIZACION'] = $output_fel["AUTORIZACION"];
		$data['SERIE'] = $output_fel["SERIE"];
		$data['NUMERO'] = $output_fel["NUMERO"];
		$data['FECHA_EMISION'] = $output_fel["FECHA_DTE"];
		$data['NIT'] = $output_fel["NIT_COMPRADOR"];
		$data['NOMBRE_CLIENTE'] = $output_fel["NOMBRE_COMPRADOR"];
		$data['DTE_LEYENDA'] = $output_fel["DTE_LEYENDA"];
	...
	public function posorderinvoice($id){
		...
		$items = $this->order_model->customerorder($id);
		foreach ($items as $key => $value){
			$data ["items"][$key]["ProductName"] = $value->ProductName . " " . $value->variantName;
			$data ["items"][$key]["precio_unidad"] = $value->mprice ;
			$data ["items"][$key]["precio_total"] = $value->mprice * $value->menuqty;
			$data ["items"][$key]["menuqty"] = $value->menuqty;
		}

		if($customerorder->cutomertype ==  1) {
            $propina_total = $grandtotal_new * 0.1;
            $key = $key +1;
    	    $data ["items"][$key]["ProductName"] = "Propina";
    	    $data ["items"][$key]["precio_unidad"] = $propina_total;
    	    $data ["items"][$key]["precio_total"] =  $propina_total;
    	    $data ["items"][$key]["menuqty"] = 1;
        }
		// Agregamos NIT
		$data["nit"] = $settinginfo->vattinno;
		$data_x = json_encode($data);
	...
	public function posprintdirect($id, $fel = null,$nit = null, $client_name = null , $detalle = true , $items = null){
		$this->permission->method('ordermanage','read')->redirect();
		$saveid=$this->session->userdata('id');
		$isadmin=$this->session->userdata('user_type');
		$customerorder=$this->order_model->read('*', 'customer_order', array('order_id' => $id));
		...
		$data['module'] = "ordermanage";
		$data['page']   = "posinvoice";   
		$data['AUTORIZACION'] = $fel["AUTORIZACION"];
		$data['SERIE'] = $fel["SERIE"];
		$data['NUMERO'] = $fel["NUMERO"];
		$data['FECHA_EMISION'] = $fel["FECHA_DTE"];
		$data['NIT'] = $fel["NIT_COMPRADOR"];
		$data['NOMBRE_CLIENTE'] = $fel["NOMBRE_COMPRADOR"];
		$data['DTE_LEYENDA'] = $fel["DTE_LEYENDA"];
	...
	public function paymultiple(){
		...
		$detallada 				 = $this->input->post('detalle',true);
		...
		$data["total"] =$this->input->post('totalamount',true);
		$data["nit_cliente"] = $nit;
		$data["nombre_cliente"] = ($client_name == "" ? "Consumidor Final" : $client_name );
		$items = $this->order_model->customerorder($orderid);
		$grandtotal_new = 0;
		$prebillinfo = $this->db->select('*')->from('bill')->where('order_id',$orderid)->get()->row();
		// $propina = $prebillinfo->service_charge;
		...
		if($detallada== "yes"){
			...
			foreach ($items as $key => $value){
				...
				$data ["items"][$key]["precio_total"] = $value->mprice * $value->menuqty;
				$data ["items"][$key]["menuqty"] = $value->menuqty;
				$grandtotal_new = $grandtotal_new+$data ["items"][$key]["precio_total"];
			}
			if($orderinfo->cutomertype ==  1) {
			 	$propina_total = $prebillinfo->service_charge;
				$key = $key +1;
				$data ["items"][$key]["ProductName"] = "Propina";
				$data ["items"][$key]["precio_unidad"] = $propina_total;
				$data ["items"][$key]["precio_total"] =  $propina_total;
			...
			$data ["items"][0]["ProductName"] = "Consumo de Alimentos";
			$data ["items"][0]["precio_unidad"] = $grandtotal_new;
			$data ["items"][0]["precio_total"] =  $grandtotal_new;
			$data ["items"][0]["menuqty"] = 1;
			
			$propina_total = $prebillinfo->service_charge;
			$key = $key +1;
			
			$data ["items"][1]["ProductName"] = "Propina";
			$data ["items"][1]["precio_unidad"] = $propina_total;
			$data ["items"][1]["precio_total"] =  $propina_total;
			$data ["items"][1]["menuqty"] = 1;
		...
		// Agregamos NIT
		$data["nit"] = $settinginfo->vattinno;
		if ( $paytype[0] != 18 && $data["nit"] != "" ) {
			$data_x = json_encode($data , true);
			$URL = "https://app.centraldepago.com/api/certificar";
			$ch = curl_init($URL);
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		...
		$data['module'] = "ordermanage";
		$data['page']   = "updateorderlist"; 
		while ($output_fel != null ){
			//echo "<pre>";var_dump($data);die;
			$view = $this->posprintdirect($orderid,$output_fel,$nit,($client_name == "" ? "Consumidor Final" : $client_name ) , $detalle_band , $data ["items"]);
			echo $view;exit;
		}
		// echo $view;exit;
	}
 ...
 public function closecashregister(){
	...
	public function kichanReport(){
		...
		$i =0;
		$findkicen = $this->order_model->kiread();
		$kichendata = array();
		$y=0;
		foreach ($findkicen as $kitchen) {
			$preports  = $this->order_model->itemsKiReport($kitchen->kitchenid,$start_date,$end_date);
			$totalamount = 0;
			$pricewithaddons =0;
	...
	public function closecashregisterprint(){
		...
		//var_dump(		$saveid=$this->session->userdata('id')	);die;
		$this->form_validation->set_rules('totalamount',display('amount'),'required');
		$saveid=$this->session->userdata('id'); 
		$counter=$this->input->post('counter');
		$openingamount=$this->input->post('totalamount');
		...
		//if ($this->form_validation->run() === true) {
			...
			if ($this->order_model->closeresister($postData)) {
				...
				$data["cocinas"] = $this->kichanReport();
