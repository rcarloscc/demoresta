...
$(document).on("keypress", '#itemqty_1', function(e){
 ...
 $("#edit").on('shown.bs.modal', function () {
  ...
  function placeorder() {
      ...
      var errormessage = '';
      if (customer_name == '') {
          errormessage = errormessage + '<span>POR FAVOR SELECCIONE CLIENTE.</span>';
          alert("PORFAVOR SELECCIONE CLIENTE");
          return false;
      }
      if (ctypeid == '') {
          errormessage = errormessage + '<span>POR FAVOR SELECCIONE TIPO DE CLIENTE.</span>';
          alert("POR FAVOR SELECCIONE TIPO DE CLIENTE!!!");
          return false;
      }
      if (isitem == '' || isitem == 0) {
          errormessage = errormessage + '<span>POR FAVOR AGREGUE COMIDAS</span>';
          alert("POR FAVOR AGREGUE COMIDAS");
          return false;
      }
      if (ctypeid == 3) {
          ...
          if (isdelivary == '') {
              errormessage = errormessage + '<span>POR FAVOR SELECCIONE TIPO DE CLIENTE.</span>';
              alert("POR FAVOR SELECCIONE EMPRESA DE REPARTO!");
              return false;
          }
          ...
          if (possetting.waiter == 1) {
              ...
              if (waiter == '') {
                  errormessage = errormessage + '<span>POR FAVOR SELECCIONE MESERO.</span>';
                  alert("POR FAVOR SELECCIONE MESERO");
                  return false;
              }
          }
      } else {
          ...
          if (possetting.tableid == 1) {
              if (tableid == '') {
                  $("#tableid").select2('open');
                  toastr.warning("POR FAVOR SELECCIONE MESA", 'Warning');
                  return false;
              }
              if (possetting.tablemaping == 1) {

                  if (tableid == '' || !$.isNumeric($('#table_person').val())) {
                      toastr.warning("POR FAVOR INDIQUE CANTIDAD DE PERSOSA", 'Warning');
                      return false;
                  }
              }
          }
      ...
      if (errormessage == '') {
          ...
          var errormessage = '<span style="color:#060;">INGRESO CON ÉXITO.</span>';
              ...
              success: function(data) {
                  ...
                  if (err == "error") {
                      ...
                      if (basicinfo.printtype == 1) {
                          ...
                          swal({
                                  title: lang.ord_succ,
                                  text: "¿ QUIERE IMPRIMIR LA COMANDA ?",
                                  type: "success",
                                  showCancelButton: true,
                                  confirmButtonColor: "#28a745",
                                  confirmButtonText: "Si",
                              ...
                              function(isConfirm) {
                                  if (isConfirm) {
                                      //window.open("http://192.168.10.31/index.php?id=" + data ,  "_blank");
                                      printRawHtml(data);
                                  } else {
                                      $('#waiter').select2('data', null);
                                      $('#tableid').select2('data', null);
          ...
          success: function(data) {
              ...
              if (value == 1) {
                  setTimeout(function() {
                      ...
                      toastr.success("Pago registrado", 'Success');
  ...
  function closeandprintcashregister() {
      ...
      $.ajax({
          ...
          success: function(data) {
              if (data ==0) {
				  alert("¡Algo salio mal con el cierre!");
              } else {
                  $.ajax({
                    type: "GET",
                    url: basicinfo.baseurl + "ordermanage/order/closecashregisterprint",
                    success: function(dataRes) {
                        setTimeout(function() {
                            ...
                            toastr.success("Cierre registrado", 'Success');
                            $("#openregister").modal('hide');
                            $('#modal-ajaxview').empty();
                            
                            window.location.href = basicinfo.baseurl + "dashboard/home?cashcloser=print";
                            prevsltab.trigger("click");
                        }, 100);
                        printRawHtml(dataRes);
                    } });
                }
            }
